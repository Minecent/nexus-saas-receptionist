  ...
  return (
    <div className={cn('px-5 py-4 flex items-start gap-4', isPending && 'bg-slate-800/20')}>
      <div className="flex-1 min-w-0">
        ...
        {item.caller_number && (
          <a href={`tel:${item.caller_number}`} className="flex items-center gap-1 text-xs text-slate-500 hover:text-teal-400 transition-colors mb-2">
            <Phone className="size-3" /> {item.caller_number}
          </a>
        )}
        <p className="text-sm text-slate-300 leading-relaxed">{item.message ?? '(no message)'}</p>
      </div>
      {isPending && (
        <button
          onClick={markHandled}
        ...
        >
          <Check className="size-3" /> {loading ? 'Saving…' : 'Mark handled'}
        </button>
      )}
    </div>
  )
