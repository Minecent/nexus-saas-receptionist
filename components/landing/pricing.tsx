    ...
    <section id="pricing" className="border-b border-slate-800 bg-slate-900">
      <div className="mx-auto max-w-7xl px-4 py-20 sm:px-6">
        ...
        <div className="mb-12 text-center">
          ...
          {/* Anchor pill */}
          <div className="mb-5 inline-flex items-center gap-3 rounded-full border border-slate-700 bg-slate-800/60 px-5 py-2">
            <span className="text-sm font-medium text-slate-400 line-through">$4,000/mo receptionist</span>
          ...
          <div className="mt-6 inline-flex items-center gap-1 rounded-full border border-slate-700 bg-slate-800 p-1">
            <button
              ...
              className={cn(
                ...
                !isAnnual ? 'bg-slate-700 text-white' : 'text-slate-400 hover:text-white'
              )}
            >
              Monthly
            </button>
            <button
              ...
              className={cn(
                ...
                isAnnual ? 'bg-teal-500 text-white' : 'text-slate-400 hover:text-white'
              )}
            >
              Annual
              <span className={cn(
                'rounded-full px-1.5 py-0.5 text-xs font-bold',
                isAnnual ? 'bg-white/20 text-white' : 'bg-slate-700 text-teal-400'
              )}>
                Save 20%
              </span>
            </button>
          ...
          {tiers.map((tier) => (
            ...
            >
              ...
              <div className="mb-4 flex flex-col gap-1">
                {tier.badge ? (
                  <>
                    ...
                    {tier.subtext && (
                      <p className="text-xs text-slate-500">{tier.subtext}</p>
                    )}
                  </>
                ) : <span />}
              </div>
              ...
              <p className={cn(
                ...
                tier.highlight ? 'text-teal-400' : 'text-slate-400',
              ...
              {tier.description && (
                <p className="mb-2 text-xs text-slate-500">{tier.description}</p>
              )}
              ...
              {tier.savingsVsReceptionist && (
                <div className="mb-3 rounded-lg border border-teal-500/20 bg-teal-500/5 px-3 py-2 text-center">
                  ...
                  <span className="text-xs text-slate-400"> vs. a receptionist</span>
                </div>
              )}
