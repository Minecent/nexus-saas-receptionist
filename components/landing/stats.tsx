...
export default function Stats() {
  return (
    <section className="relative border-b border-slate-500 bg-slate-700">
      ...
      <div className="relative mx-auto max-w-7xl px-4 pt-16 pb-14 sm:pt-28 sm:pb-20 sm:px-6">
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-3">
          {stats.map((stat) => (
            <div
              key={stat.title}
              className="rounded-2xl border border-slate-500 bg-slate-600 p-6"
            >
              ...
              {stat.source && (
                <p className="mt-3 text-xs text-slate-300">Source: {stat.source}</p>
              )}
            </div>
          ))}
        </div>
